...
(X_train, y_train), (X_test, y_test) = imdb.load_data( get_file_path, num_words=max_features )
...
for mode in modes:
    ...
    model.compile(loss='binary_crossentropy',# 损失函数为二进制分类的
                  optimizer='adam',  # 优化函数是自适应
                  metrics=['accuracy'])# 评价标准是准确度
